...
class Search:
    ...
    def cluster_extend(self, D, k, doc_length):
        ...
        for file_id in file_id_arr:
            ...
            doc_id2score = {}
            with open(IR.conf.data_cat_path + str(file_id), "r")as input_file:
    ...
    def star_arrange(self, D, k, doc_length):
        ...
        doc2star = {}
        for doc_id in result:
            line = linecache.getline(IR.conf.data_path + IR.conf.id2datafile[str(doc_id)[0]],
                                     doc_id % IR.conf.magnitude)
            json_obj = json.loads(line.strip())
